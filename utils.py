...
def _variable_on_cpu(name, shape, initializer, use_fp16=False):
  ...
  Args:
    ...
    shape: list of ints
    initializer: initializer for Variablew
  Returns:
    Variable Tensor
  """
...
def unpool_with_with_argmax(pooled, ind=None, ksize=[1, 2, 2, 1]):
    """
    ...
    # The indices in argmax are flattened, so that a maximum value at position [b, y, x, c] becomes flattened index (
    # (b * height + y) * width + x) * channels + c Create a single unit extended cuboid of length bath_size
    # populating it with continous natural number from zero to batch_size
...
def dense(x, inputFeatures, outputFeatures, scope=None, with_w=False):
    with tf.variable_scope(scope or "Linear"):
        ...
        if with_w:
            ...
            return tf.nn.relu(tf.matmul(x, matrix) + bias)
